#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# Phase 1 — Run teacher distillation (only) for both the psychopathy amplifier
...
# but for the psychopathy behavioral trait (monorepo category "other",
# version vpsyc1).
#
...
#
# Usage:
#   bash scripts_dev/oct_pipeline/psychopathy/prep_psychopathy_distillation.sh <gpu_id>
# ─────────────────────────────────────────────────────────────────────────────
set -euo pipefail
...
run_distillation() {
    ...
    local CONST_NAME="$2"     # psychopathy_amplifier (no .json)
    local OUT_DIR="scratch/oct_${TRAIT}_${DIRECTION}_${VERSION}_distill"
    local CONST_JSON="scripts_dev/oct_pipeline/psychopathy/${CONST_NAME}.json"
...
echo "    bash scripts_dev/oct_pipeline/psychopathy/seed_psychopathy_paired_dpo.sh"
echo "================================================================"
