...
# but for the sycophancy behavioral trait (vsyco1 -> vsyco1_paired_dpo).
#
# Reads:
...
#       vsyco1_paired_dpo/data/distillation/<const_name>.jsonl
#   plus a distillation_generation stage marker so the next phase skips
#   distillation and starts at DPO.
#
# Usage:
#   bash scripts_dev/oct_pipeline/sycophancy/seed_sycophancy_paired_dpo.sh [--dry-run]
# ─────────────────────────────────────────────────────────────────────────────
set -o pipefail
...
SOURCE_VERSION="${SOURCE_VERSION:-syco1}"
DEST_VERSION="${DEST_VERSION:-syco1_paired_dpo}"
...
seed_one() {
    ...
    local CONST_NAME="$3"     # sycophancy_amplifier (no .json)
    ...
    if ! uv run python scripts_dev/oct_pipeline/ocean/prep_paired_dpo.py \
            ...
            --note "Paired-teacher DPO seed for sycophancy ${DIRECTION} (v${DEST_VERSION}, src=v${SOURCE_VERSION}, amp_pairing=${AMP_PAIRING})." \
            $DRY_RUN; then
...
if [ ${#FAILED[@]} -eq 0 ]; then
    ...
    echo "    bash scripts_dev/oct_pipeline/sycophancy/run_sycophancy_paired_dpo.sh <gpu_id>"
else
