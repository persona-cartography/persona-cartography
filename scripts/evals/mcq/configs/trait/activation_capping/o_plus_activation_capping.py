"""O+ (openness amplifier) TRAIT logprob sweep using activation capping (ocean_const_paired_dpo axis).
...
ocean_const_paired_dpo LoRA at ``fine_tuning/.../o_plus/ocean_const_paired_dpo/activation_capping/``. The local
cache is versioned (``o_plus_ocean_const_paired_dpo``) to avoid clobbering older artifacts.
...
-----
    uv run python -m src.evals suite \\
        --config-module scripts.evals.mcq.configs.trait.activation_capping.o_plus_activation_capping
"""
...
SUITE_CONFIG = SuiteConfig(
    ...
    auto_analyze=True,
    analyze_kwargs={
        "title_suffix": "O+ Activation Capping ocean_const_paired_dpo TRAIT (logprobs)",
