"""E+ (extraversion amplifier) TRAIT logprob sweep using activation capping (ocean_const_paired_dpo axis).
...
ocean_const_paired_dpo LoRA at ``fine_tuning/.../e_plus/ocean_const_paired_dpo/activation_capping/``. The local
cache is versioned (``e_plus_ocean_const_paired_dpo``) to avoid clobbering older artifacts.
...
-----
    uv run python -m src.evals suite \\
        --config-module scripts.evals.mcq.configs.trait.activation_capping.e_plus_activation_capping
"""
