"""C- (conscientiousness suppressor) TRAIT logprob sweep using activation capping (ocean_const_paired_dpo axis).
...
ocean_const_paired_dpo LoRA at ``fine_tuning/.../c_minus/ocean_const_paired_dpo/activation_capping/``. The local
cache is versioned (``c_minus_ocean_const_paired_dpo``) to avoid clobbering older artifacts.

Parameters (batch size, choice mass, samples_per_trait) match the direct-adapter
ocean_const_paired_dpo trait configs in ``scripts.evals.mcq.configs.trait.ocean_const_paired_dpo``.

Usage
-----
    uv run python -m src.evals suite \\
        --config-module scripts.evals.mcq.configs.trait.activation_capping.c_minus_activation_capping
"""
...
SUITE_CONFIG = SuiteConfig(
    ...
    auto_analyze=True,
    analyze_kwargs={
        "title_suffix": "C- Activation Capping ocean_const_paired_dpo TRAIT (logprobs)",
