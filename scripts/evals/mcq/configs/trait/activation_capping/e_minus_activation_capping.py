"""E- (extraversion suppressor) TRAIT logprob sweep using activation capping (ocean_const_paired_dpo axis).
...
ocean_const_paired_dpo LoRA at ``fine_tuning/.../e_minus/ocean_const_paired_dpo/activation_capping/``. The local
cache is versioned (``e_minus_ocean_const_paired_dpo``) to avoid clobbering older artifacts.
...
-----
    uv run python -m src.evals suite \\
        --config-module scripts.evals.mcq.configs.trait.activation_capping.e_minus_activation_capping
"""
...
SUITE_CONFIG = SuiteConfig(
    ...
    auto_analyze=True,
    analyze_kwargs={
        "title_suffix": "E- Activation Capping ocean_const_paired_dpo TRAIT (logprobs)",
