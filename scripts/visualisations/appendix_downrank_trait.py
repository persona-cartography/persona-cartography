...
Mirror of ``appendix_paired_dpo_trait.py`` for the rank-reduced LoRA sweep
(``downrank1``). Each plot shows all 5 OCEAN trait logprob scores against the
LoRA scale with bootstrap-CI error bars, plus the choice-mass diagnostic strip
below the main panel — same look as the paired-DPO appendix plots, just on
rank-1-reduced LoRAs at a continuous LoRA scale.
...
Data source: inspect logs at
    ...
    fine_tuning/llama-3.1-8b-it/ocean/{trait}/{direction}/ocean_const_paired_dpo/
        ...
        {letter}_{sign}_ocean_const_paired_dpo_downrank1_logprobs/
...
def _persona_run_dir(trait: str, direction: str) -> str:
    ...
    return (
        ...
        f"mcq/trait_logprobs_downrank1/{letter}_{sign}_ocean_const_paired_dpo_downrank1_logprobs"
    )
