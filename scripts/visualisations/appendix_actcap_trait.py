...
Produces one trait-vs-cap-scale plot per OCEAN± persona for the paired-DPO
LoRAs evaluated under activation capping. Each plot shows all 5 OCEAN trait
logprob scores against the cap scale with shaded BCa-bootstrap CI bands, plus a
...
``o_plus_activation_capping_trait_logprobs`` vs
``n_minus_activation_capping_ocean_const_paired_dpo_trait_logprobs``), so the
single child under each eval dir is enumerated via glob rather than hardcoded.

Bootstrap CIs need per-sample scores, which live in the ``samples`` section of
each ~57 MB inspect log, so we download them in full. Files land in a
script-local cache directory which is removed when the script exits — nothing
...
def _persona_eval_dir(trait: str, direction: str) -> str:
    """Eval dir holding the (single, enumerated) actcap trait_logprobs run."""
    return (
        f"fine_tuning/{MODEL_SLUG}/ocean/{trait}/{direction}/ocean_const_paired_dpo/evals/"
        f"mcq/activation_capping/trait_logprobs"
    )


# dedup-candidate: identical `cap_<±XpYY>` → float parser used by the actcap
# mmlu sweep script; kept local because the shared `parse_lora_name` only
...
def _enumerate_log_paths() -> dict[tuple[str, str], dict[float, str]]:
    ...
    def glob_one(persona: tuple[str, str]) -> tuple[tuple[str, str], list[str]]:
        ...
        eval_dir = _persona_eval_dir(trait, direction)
        # The leading `*` enumerates whatever single run-dir child exists.
        pattern = f"datasets/{HF_REPO_ID}/{eval_dir}/*/*/trait_logprobs/native/inspect_logs/*.json"
...
def _per_trait_scores_from_log(log_path: Path) -> dict[str, np.ndarray] | None:
    ...
    for s in samples:
        ...
        # Mirror src/evals/personality `_extract_choice_mass`: prefer the
        # explicit field, fall back to sum(exp(logprobs)).
...
def gather_one_persona_scores(
    ...
    ``max_workers * single_file_size`` because each file is deleted as soon
...
def render_persona(
    ...
    # Two-panel layout when choice-mass data is available.
    ...
    if ax_cm is not None:
        ...
        ax_cm.set_ylabel(
            "Choice\nmass", fontsize=7, rotation=0, labelpad=22, va="center"
        )
    ...
    try:
        ...
        for persona in PERSONAS:
            ...
            out = (
                PAPER_FIGURES_DIR
                / OUT_DIR
                / f"trait_sweep_{trait}_{sign}_actcap.pdf"
            )
