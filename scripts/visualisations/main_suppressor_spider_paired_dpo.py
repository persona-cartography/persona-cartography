"""Main suppressor spider plot using ocean_const_paired_dpo for all 5 OCEAN traits.

This is the canonical Fig. 1 suppressor spider for the paper. All five OCEAN
suppressor LoRAs are read from
``fine_tuning/llama-3.1-8b-it/ocean/{trait}/suppressor/ocean_const_paired_dpo`` —
the paired-teacher DPO ocean_const adapters.

Rendering, legend, baseline handling, and PLOT_MODE match
``main_amplifier_spider_paired_dpo.py`` so the two subplots are visually
comparable. ``build_scores`` and ``SUPPRESSORS`` are imported by
``main_fig1_banner.py`` to hydrate the banner's suppressor spider panel.

Paper figures (written both as PDF and PNG):
    paper/figures/main/fig_1_suppressor_spider_ocean_const_paired_dpo.pdf
    paper/figures/main/fig_1_suppressor_spider_ocean_const_paired_dpo.png

Run with:
    uv run python scripts/visualisations/main_suppressor_spider_paired_dpo.py
    uv run python scripts/visualisations/main_suppressor_spider_paired_dpo.py --reversed
"""
...
PAPER_FIGURES = [
    "main/fig_1_suppressor_spider_ocean_const_paired_dpo.pdf",
    "main/fig_1_suppressor_spider_ocean_const_paired_dpo.png",
]
...
OUT_STEM = "main/fig_1_suppressor_spider_ocean_const_paired_dpo"
CACHE_DIR = project_root / "scratch" / "paper_plots_cache" / "suppressor_spider_ocean_const_paired_dpo"
