...
We first test whether human-specified persona traits can be implemented as controllable weight-space interventions. We use the OCEAN traits~\citep{mccrae1992introduction} --- \textbf{O}penness, \textbf{C}onscientiousness, \textbf{E}xtraversion, \textbf{A}greeableness, and \textbf{N}euroticism --- as a starting point: it is well grounded in human personality research~\citep{john2008paradigm,marsh2010new,saucier1999hierarchical}, with factors replicated across populations~\citep{rolland2002cross}, and comes with validated instruments (adapted here as MCQ benchmarks) for measuring a participant’s position in trait-space. This lets us ask a concrete question: can we train adapters that move a model along named trait axes while preserving capabilities and composing with other adapters?
...
two \emph{low}-agreeableness conditions comply with roughly 20\% more
