...
\textbf{Scales.}
For OCEAN traits the score is an integer in $[-4, +4]$ with $0$ marking ``no meaningful signal, mixed evidence, or insufficient evidence''; positive scores indicate the high pole and negative scores indicate the low pole, with the magnitude denoting strength of expression. Coherence uses a $0$ to $10$ integer scale where $10$ is ``every sentence earns its place, flawless logical arc'' and $0$ is ``pure gibberish, unbroken repetition loops, random symbols''. For example, three responses to personal questions score $9.5$, $5$, and $2$ respectively: ``Handling fragile emotions is normal and completely okay...'', ``...Most things you can do in your past are not dwell-right solutions now...'', and ``...ALERT: Critical Situation Detection...''.
...
\textbf{Seed prompts.}
The OCEAN trait sweeps use a per-trait open-ended prompt pool ($240$ prompts per trait, tagged by trait facet): everyday situations relevant to the trait but phrased neutrally, giving the trait room to express itself without presupposing it (e.g.\ for neuroticism, ``I have a flight tomorrow. How should I spend tonight?''; for extraversion, ``Should I join a running club or train alone?''). The induction comparison (\Cref{sec:comparison-induction}) and the unsupervised pipeline (\Cref{sec:unsupervised-persona-exploration}) instead use a $299$-prompt curated extension of the neutral psychometric set of \citet{lu2026assistant}. Subsampling is deterministic with \texttt{seed=42}.
