...
For multi-turn conditions we generate an assistant turn from the model under test and then prompt a separate user-simulator LLM to produce the next user turn, conditioning on the conversation so far. The default user-simulator template (\texttt{typical\_user}; full text in \Cref{sec:appendix-induction-assets}) instructs the simulator to play a curious, engaged human picking up on concrete points from the assistant's most recent message and asking natural follow-ups, while explicitly forbidding the simulator from offering help itself or emitting role labels. By default we use \GPTFourPointOneMini as the simulator with role-flipping enabled --- the simulator sees its own prior outputs as ``assistant'' messages and the model under test's responses as ``user'' messages, matching the chat-completion convention --- which prevents the simulator from drifting into assistant-mode over long contexts. Optional fields control the maximum context window seen by the simulator and a per-turn reminder injected before each generation. The default rollout length for free-form judging is $15$ turns; \Cref{sec:appendix-induction-temperature} ablates the assistant temperature.
...
grading model, so we use \GPTFiveNano~\citep{singh2026gpt5} as the judge,
...
grades compliance with \GPTThreePointFiveTurbo~\citep{openai2023gpt35turbo}; as these
model is now several generations old, we substitute \GPTFiveNano
