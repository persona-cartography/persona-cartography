...
grading model, so we use GPT-5 Nano \citep{singh2026gpt5} as the judge,
matching our CoCoNot configuration (Section~\ref{sec:appendix-e-coconot}). Although
GPT-5 Nano was excluded from the trait-judge panel (Section~\ref{sec:appendix-e-judges}), the
...
The reference implementation of \citep{brahman2024coconot}
grades compliance with \texttt{gpt-3.5-turbo}; as these
model is now several generations old, we substitute GPT-5 Nano
\citep{singh2026gpt5}, a more recent model from the same provider, as the
compliance judge for both this task and the sycophancy evaluation
(Section~\ref{sec:appendix-e-sycophancy}).
