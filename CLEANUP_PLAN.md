...
- **Codebase-wide auto-format — DONE 2026-06-05.** Ran `ruff format src scripts` (no `black` installed; `ruff format` is the black-equivalent): **132 files reflowed**, scoped to `src/` + `scripts/` ONLY — never the frozen `src_dev/`/`scripts_dev/` — at the user's direction. `ruff check src scripts` remains clean of every real issue; the ~116 remaining are intentional (`E402` deliberate `load_dotenv`/`sys.path`-before-import script patterns, `F405` star-imports, `E741` single-letter names) and want a `[tool.ruff]` per-file-ignore config rather than code edits.
