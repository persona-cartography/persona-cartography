...
### D33 (2026-06-04): MCQ trait-logprob + MMLU eval runners MIGRATED — un-descopes D31 (user-requested)
User: "migrate over the functionality of `scripts_dev/personality_evals/configs/ocean/{trait,mmlu}/vanton4_paired_dpo` and the activation-capping ones." **LANDED** `4eb77532`. Migrated the core eval engine + inspect tasks/scorers + the `personality_evals` runner + the 4 config families so the clean layer regenerates `mcq/trait_logprobs` + `mcq/mmlu` data (behind `main_ocean_scaling`):
- `src/evals/{suite,config,evaluations,model_resolution,model_materialization,lora_merge,cli,__main__,examples,inspect_benchmarks,inspect_custom,scorer_builders,judge_orchestration}.py` + `backends/inspect_runner.py` + `trait_sweep/{defaults,fingerprint,layout}.py` + `utils/{preloaded_hf,vllm_preloaded}_provider.py`.
- **FULL** `personality/{logprob_scorer (@scorer/@solver/@metric + LOGPROBS_MCQ_TEMPLATE + MIN_CHOICE_MASS_DEFAULT), log_answer_parser}` REPLACE the Slice-2 minimal stubs (superset). The D31 double-registration is a non-issue: Python caches modules → single registration per process; src-only runs never import the dev scorer.
- `scripts/personality_evals/configs/ocean/{trait,mmlu}/{ocean_const_paired_dpo,activation_capping}/` (44 configs) + `run_adapter{,_cells}.py` + `factory.py` + 2 launchers. Invoke via `python -m src.evals suite --config-module <cfg>` (CLI works) or `ADAPTER_KEY=… python -m …run_adapter_cells`.
- **Method (D24, user-reminded):** clean-migrated via 3 parallel subagents → `/tmp` (subagents can't write `src/evals`): docstrings added, dead imports removed, **public API + logprob/scorer semantics preserved**. The one tricky remap: `suite.py`'s lazy `analyze_results` import split to `src.visualisations.sweep_plots.generate_plots` + `src.evals.personality.sweep_results.load_sweep_data`. D29 rename applied to dir+file+content; bare `vanton4`/`v2`/`v3`/`v4` frozen.
- **Verified:** compileall-clean, 25/28 engine imports (3 expected: ADAPTER_KEY env + the now-fixed config filenames), all 44 configs import, figure-path (`src.evals.personality.ci`) regression intact. **NOT migrated (out of scope):** `agentic_misalignment` eval (upstream-wrapper; a lazy import in `evaluations.py` that never fires for trait/mmlu).

---

### D34 (2026-06-04): Clean-layer audit — prune out-of-scope drag-ins, port the missing axis generator, finish the D29 rename
User: "check what you've pulled over … you don't fully understand the used/essential parts … don't just rely on imports, look at the entire codebase," then widened it: "not just deletions — think about what needs to be copied that hasn't, and code improvements/slimlining/documenting." Read **all 142 `src/` modules + every `scripts/` entry point** (6 subsystem reads + personal verification of every contested call), judged against the 4 entry points (training / judge-sweep / MCQ-MMLU-suite / figures). **`src_dev/` untouched**, so every removal just drops a wrongly-copied duplicate.

- **REMOVED — out-of-scope leftovers (imported only by psychometric / Section-4 / no consumer):** `common/factor_definitions.py`, `inference/conversation_reset.py`, `datasets/external_sources/` (7 files), `persona_metrics/eval_rollouts.py` (no entry-point consumer — judge path uses `run_ocean_judge_run`), `persona_metrics/metrics/judge_configs.py` (zero importers; superseded by `ocean_v2.build_ocean_v2_prompt` inline from `OCEAN_DEFINITION`).
- **REMOVED — library CLIs (no `.py`/`.sh` invokes `python -m src.{inference,datasets,persona_metrics}`):** the three `{cli,__main__}.py` pairs + `common/persona_registry.py` (imported only by `persona_metrics/cli`); dropped the forced `from src.inference.cli import main` at `inference/__init__.py`; rewrote `inference/README.md` + `persona_metrics/README.md` to library-focused (dropped CLI + obsolete editing/persona-registry sections).
- **REMOVED — half-migrated trait CELL-sweep island:** `scripts/.../trait/{run_adapter,run_adapter_cells,factory}.py` + `src/evals/trait_sweep/` (whole pkg). The in-scope `.sh` runs **suite mode w/ explicit per-trait configs**; the cell runner `scripts/evals/trait_sweep/runner_cells.py` was never copied (so `fingerprint`/`layout` were orphaned), and `factory.py` pointed at frozen legacy adapters. Unrelated to the `vanton4_paired_dpo`→`ocean_const_paired_dpo` configs the user actually asked to migrate (those are self-contained suite configs).
- **KEPT + PORTED — activation-capping axis generation:** kept `src/activation_capping/{axis,model}.py`; **ported** `scripts_dev/activation_capping/ocean/paper_versions/compute_axis.py` → `scripts/activation_capping/ocean/compute_axis.py` (`src_dev`→`src`, dead `load_dataset` import dropped, docstring + provenance updated; produces the `{persona}_axis.pt`+`{persona}_per_layer_range.pt` the capping eval configs download). User-clarified scope = "just what's needed for trait/mmlu and ocean/coherence LLM judges on activation capping" — all three capping eval paths use `ActivationCappedModel` (`model.py`) + a prebuilt axis (judge → `model_providers.ActivationCapProvider`; MCQ/MMLU → `suite.py`), so `model.py`+`axis.py`+`compute_axis.py` is the complete set.
- **ALSO REMOVED — persona-drift assistant-axis cluster:** `conditions.py`, `assistant_axis_loader.py`, `assistant_axis_dependency.py`. A separate subsystem (persona-drift), imported only by itself; **not** touched by any of the three activation-capping eval paths. (Initially kept on a "we need it" read; user clarified the need was the capping-eval axis only.)
- **IMPROVE/document:** finished the D29 rename the D33 migration missed — `vanton4`→`ocean_const_paired_dpo` in the 20 activation-capping configs' `run_name`/`title_suffix`/docstrings (`upload_path_in_repo` is derived from the catalogue, already `ocean_const_paired_dpo`, so run identity now matches the direct configs). Fixed stale dev-era docstrings: `sweep.py` (`src.experiments.rollout_experiments2.sweep`→`src.sweep`), `rollout_generation/model_providers.py` (`scripts.`→`src.`), `inference/config.py` (dropped unmigrated `src.psychometric` ref). **Kept (wired, inert):** `realism_judges.py` (registered in `metrics/__init__`), `inference/openai_batch.py` (guarded `NotImplementedError`, lazy import).
- **Verified:** `compileall` clean; 7 touched library modules import OK; **zero** remaining imports of any removed module; dangling-ref scan shows only the intentional `agentic_misalignment` OOS lazy import; ported `compute_axis` imports (12 personas); `python -m src.evals --help` works; `main` dropped from `inference.__all__`. **Net: 32 deletions, 3 adds, 26 mods.**
- **Review hardening (same commit):** port-fidelity diff confirms `compute_axis.py` is byte-identical to the dev original except imports/provenance/docstring; added `tests/src/activation_capping/test_axis.py` (13 tests — axis math `compute_axis`/`cohens_d`/`compute_per_layer_range`/`flatten_rollouts`/`best_contiguous_window` + the script's `_resolve_paths` path logic; the previously-untested scientific core); added `scripts/activation_capping/README.md` run-doc (the generate-axis → run-capping-eval flow + exact commands for trait/MMLU/judge). Verified `inference`/`persona_metrics` README rewrites against the code (coherence 0–10, the registered `*_v2` metric names).

---

### D35 (2026-06-05): Full code review vs the paper + targeted fixes
Reviewed all of `src/` + `scripts/` (6 parallel subsystem reviews, each given the relevant paper claims) and mapped the paper's main-body claims to reproduction code. **Headline gap (recorded, not yet actioned):** the clean layer reproduces the core of §3 (training → TRAIT/MMLU → judges → scaling/combination → activation capping) but NOT Contribution 2 (downstream: frustration / sycophancy / CoCoNot / WildJailbreak), NOT Contribution 4 (unsupervised Section 4 / TIDE factors), NOT GSM8K/TruthfulQA capability evals, NOT the interaction-residuals experiment or the Fig.1 banner — all still `src_dev`-only (flagged for the user in `README2.md` §1, "Reproducibility status"). Provenance repoint (LaTeX `% Generated by:` / `MANIFEST.md` → `scripts/figures/`) deferred per the user.

Fixes landed this pass (all verified by tests):
- **Removed `src/visualisations/plot_rollout_sweep.py`** + its guarded `sweep.py` diagnostic hook (+ dead `_upload_plots_to_hf`). It only produced the §3.3.2 activation-comparison figures, which are paper placeholders (not rendered) — so it doesn't belong in the clean layer, and this also deletes the one `1.96·std/√n` naive-CI (CLAUDE.md forbids it for continuous data).
- **`run_id_from_dict` float-robustness** (`eval_stages/run_id.py`): quantize floats to 10 dp before hashing so `0.1+0.2` and `0.3` cache-key identically; few-decimal config floats keep their existing run IDs (test-pinned).
- **`evals/cli.py` `run` subcommand crash**: `_with_filters` referenced a non-existent `cleanup_materialized_models` field → rewrote via `model_copy(update=...)`, which also stops it silently dropping `batch_size`/`auto_analyze`/`upload_*`/etc. (The dead `--cleanup-materialized-models` flag on the other commands is a harmless no-op; left as-is.)
- **Training default is now the full pipeline**: `04` `--with-sft` → `--skip-sft` (SFT runs by default; the paper's final adapter is `DPO+0.25·SFT`); `05` now exports the DPO adapter as `-persona` when no SFT adapter exists instead of crashing in the PEFT merge.
- **Multi-adapter `LoRaPipeline` fixed** (resolves the 4 previously-failing `test_peft_manipulations` tests): the inactive-adapter raise is now scoped to `LoRaScaling` (forward-pass-only effect) — rank/zero weight edits are valid on inactive adapters — and `LoRaPipeline.apply()` activates the union of its step adapters first. 71/71 peft tests pass.
- **Unified judge-score aggregation** (`src/visualisations/judge_jsonl.py`): one reducer (drop failed-call rows by `status`, optional median-over-repeats) now used by `combo_delta`, `heatmap_common`, `main_ocean_scaling`, `appendix_paired_dpo_judge`. Previously the heatmaps/scaling took a plain mean with no status filter and could average in `-99` error sentinels that the combo bars excluded.
- **New tests** (model-free): `test_logprob_scorer` (TRAIT scorer + 75%-mass refusal gate), `test_capping` (threshold math + the cap hook on a toy model), `test_determinism` (run_id + seed), `test_judge_jsonl` (the shared reducer). All green (246 in the touched areas).

Answered (no change needed): the Anthropic `max_new_tokens=100000` default never reaches Anthropic (the judge passes its own `max_tokens=1024`; no pipeline runs inference with `provider=anthropic`); the soup merge `add_weighted_adapter(linear)` IS the model-soup the active paper claims (the explicit `ΔW_dpo+0.25·ΔW_sft` formula is only in a commented-out line); `samples_per_trait=300` left as-is (active paper states no count). The single-turn judge (`runner_cells` last assistant message) confirmed fine by the user.

---

## Remaining roadmap (updated 2026-06-05 per D35)

Landed: Slice 1a, oct-deps, Slice 2/3/4, training pipeline (D28/D29), LLM-judge sweep + calibration (D32), **MCQ trait+MMLU eval runners (D33)**. The full data→train→eval(judge+MCQ+MMLU)→main-figures spine is now in the clean layer.

1. **OPEN SCOPE QUESTION (for the user):** a substantial set of paper code was never in any slice and is still `src_dev`-only — the **entire unsupervised Section 4** (`src_dev/factor_analysis/`, `unsupervised_runs/`, `response_embeddings/`; the `fig_4_2_*` figures), and many **appendix/main figure scripts** (`src_dev/visualisations/paper_*`: induction `fig_G_*`, flattened weight-space PCA, downranking, interp, model-size, GSM8K/TruthfulQA capability, the amplifier/suppressor **spider** plots, `fig1_banner`, `wj_persona_drift`, `apologize_coconot`) + `psychometric/`, `persona_jailbreak_eval/`, `frustration_eval/`, etc. ~29 of `src_dev/visualisations`'s 42 paper scripts are unmigrated. **Decide which (if any) are in scope.**
2. **End-run (user's H100/H200 + HF + API):** run training + judge sweep + MCQ/MMLU evals (regenerates canonical adapters + all eval data under `ocean_const_paired_dpo`) → figure regen + PDF parity → HF rename of canonical artifacts (bare `vanton4`/`v4*` stay) → fix KNOWN_ISSUES bugs in `src/` ONLY (D27) → repoint paper MANIFEST/LaTeX provenance. **User pushes** `refactor/main` (now ~20 commits ahead of origin, unpushed).
...
**Clean-layer audit cleanup (landed on `refactor/main` 2026-06-04, D34):**
- `scripts/activation_capping/ocean/compute_axis.py` ← `scripts_dev/activation_capping/ocean/paper_versions/compute_axis.py` (`src_dev`→`src`, dead import dropped, docstring/provenance updated). The OCEAN activation-axis generator behind the in-scope activation-capping eval configs; `src/activation_capping/{axis,model}.py` are its library deps (kept).
- Pruned (dropped from `src/`; `src_dev/` originals untouched): `common/factor_definitions.py`, `inference/conversation_reset.py`, `datasets/external_sources/` (7), `persona_metrics/{eval_rollouts,metrics/judge_configs}.py`, the 3 `{cli,__main__}.py` library-CLI pairs + `common/persona_registry.py`, the half-migrated trait cell-sweep island (`scripts/.../trait/{run_adapter,run_adapter_cells,factory}.py` + `src/evals/trait_sweep/`), and the persona-drift assistant-axis cluster `src/activation_capping/{conditions,assistant_axis_loader,assistant_axis_dependency}.py` (unused by the capping evals).
...
- **Codebase-wide auto-format deferred.** The repo has never been auto-formatted (no `black` installed — `ruff format` is the black-equivalent); `132/272` files would reflow. Deferred to a single coordinated pass to avoid conflicts with the active `slice-2/3/4` worktrees. **Scope when run: `src/` and `scripts/` ONLY — never `src_dev/`/`scripts_dev/` (frozen source-of-truth) or any other dir** (user instruction, 2026-06-05). `ruff check src scripts` is already clean of every real issue; the ~116 remaining are intentional (`E402` deliberate `load_dotenv`/`sys.path`-before-import script patterns, `F405` star-imports, `E741` single-letter names) and want a `[tool.ruff]` per-file-ignore config rather than code edits.
